...
describe('NgtToastService', () => {
  ...
  let mockComponentRefs: Array<Partial<ComponentRef<NgtToast>>>;
  ...
  beforeEach(() => {
    ...
    mockViewContainerRef = {
      createComponent: vi.fn(() => {
        const mockComponentRef: Partial<ComponentRef<NgtToast>> = {
          instance: {
            toastId: '',
            text: '',
            severity: 'info',
        ...
        mockComponentRefs.push(mockComponentRef);
        return mockComponentRef as ComponentRef<NgtToast>;
      })
    };
  ...
  describe('clear', () => {
    it('should clear all toasts', () => {
      ...
      vi.advanceTimersByTime(300);
      expect(mockComponentRefs[0]?.destroy).toHaveBeenCalled();
      expect(mockComponentRefs[1]?.destroy).toHaveBeenCalled();
    });
  });
});
