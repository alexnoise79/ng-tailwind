...
describe(AlertPage.name, () => {
  ...
  describe('Alert Variants', () => {
    ...
    it('should have correct aria-live attributes', () => {
      ...
      cy.get('ngt-alert').contains('Error:').closest('ngt-alert').find('div[role="alert"]').should('have.attr', 'aria-live', 'assertive');
      cy.get('ngt-alert').contains('Warning:').closest('ngt-alert').find('div[role="alert"]').should('have.attr', 'aria-live', 'assertive');

      // Others should be polite
      cy.get('ngt-alert').contains('Info:').closest('ngt-alert').find('div[role="alert"]').should('have.attr', 'aria-live', 'polite');
      cy.get('ngt-alert').contains('Success!').closest('ngt-alert').find('div[role="alert"]').should('have.attr', 'aria-live', 'polite');
    });
  });
  ...
  describe('Dismissible Functionality', () => {
    it('should show close button for dismissible alerts', () => {
      ...
      cy.get('ngt-alert').contains('This alert can be dismissed').parent().find('button[aria-label="Close alert"]').should('be.visible');
    });
    ...
    it('should not show close button for non-dismissible alerts', () => {
      ...
      cy.get('ngt-alert').contains('This alert cannot be dismissed').parent().find('button[aria-label="Close alert"]').should('not.exist');
    });
    ...
    it('should dismiss alert when close button is clicked', () => {
      ...
      cy.get('ngt-alert').contains('This alert can be dismissed').parent().should('be.visible').find('button[aria-label="Close alert"]').click();

      // Wait for animation and verify alert is hidden
      cy.wait(350);
      cy.get('ngt-alert').contains('This alert can be dismissed').should('not.exist');
    });
  });
  ...
  describe('Code View Toggle', () => {
    it('should toggle code view for demos', () => {
      ...
      cy.contains('All Variants').parent().find('button[title="Toggle code view"]').click();
